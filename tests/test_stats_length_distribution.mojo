...
def test_length_dist_init_empty():
    var ld = LengthModule()
    assert_equal(len(ld.collector.length_vector), 0)
...
def test_length_dist_tally_single_read():
    ...
    ld.collector.tally_read(rec)
    assert_equal(len(ld.collector.length_vector), 4)
    assert_equal(ld.collector.length_vector[3], 1)  # index N-1 = 3
...
def test_length_dist_tally_slot_zero():
    ...
    ld.collector.tally_read(rec)
    assert_equal(ld.collector.length_vector[0], 1)
...
def test_length_dist_tally_multiple_lengths():
    ...
    ld.collector.tally_read(rec2)
    ld.collector.tally_read(rec4)
    assert_equal(ld.collector.length_vector[1], 1)
    assert_equal(ld.collector.length_vector[3], 1)
    assert_equal(ld.collector.length_vector[0], 0)
    assert_equal(ld.collector.length_vector[2], 0)
...
def test_length_dist_tally_same_length_accumulates():
    ...
    ld.collector.tally_read(rec)
    ld.collector.tally_read(rec)
    assert_equal(ld.collector.length_vector[3], 3)
...
def test_length_dist_tally_shorter_then_longer():
    ...
    ld.collector.tally_read(short_rec)
    ld.collector.tally_read(long_rec)
    assert_equal(len(ld.collector.length_vector), 8)
    assert_equal(ld.collector.length_vector[1], 1)   # length 2 → slot 1
    assert_equal(ld.collector.length_vector[7], 1)   # length 8 → slot 7
...
def test_length_average_uniform():
    ...
    var avg = ld.collector.length_average(2)
...
def test_length_average_single_read():
    ...
    ld.collector.tally_read(rec)
    var avg = ld.collector.length_average(1)
...
def test_get_size_distribution_small_range():
    var ld = LengthModule()
    starting, interval = ld.summarizer.get_size_distribution_(0, 10)
...
def test_get_size_distribution_medium_range():
    var ld = LengthModule()
    starting, interval = ld.summarizer.get_size_distribution_(0, 51)
...
def test_get_size_distribution_nonzero_start():
    var ld = LengthModule()
    starting, interval = ld.summarizer.get_size_distribution_(100, 200)
...
def test_get_size_distribution_interval_nonnegative():
    var ld = LengthModule()
    starting, interval = ld.summarizer.get_size_distribution_(0, 1000)
...
def test_length_dist_status_fail_zero_length():
    ...
    ld.collector.tally_read(rec)
    ld.summarizer.feed_(ld.collector)
    ld.summarizer.summerize(SummaryContext(1, 0, ""))
    assert_equal(ld.summarizer._get_status(), "fail")
...
def test_length_dist_status_warn_multiple_lengths():
    ...
    ld.collector.tally_read(rec4)
    ld.summarizer.feed_(ld.collector)
    ld.summarizer.summerize(SummaryContext(2, 6, ""))
    assert_equal(ld.summarizer._get_status(), "warn")
...
def test_length_dist_status_pass_single_length():
    ...
    ld.collector.tally_read(rec)
    ld.summarizer.feed_(ld.collector)
    ld.summarizer.summerize(SummaryContext(2, 8, ""))
    assert_equal(ld.summarizer._get_status(), "pass")
...
def test_length_dist_status_pass_single_length_one_base():
    ...
    ld.collector.tally_read(rec)
    ld.summarizer.feed_(ld.collector)
    ld.summarizer.summerize(SummaryContext(1, 1, ""))
    assert_equal(ld.summarizer._get_status(), "pass")
